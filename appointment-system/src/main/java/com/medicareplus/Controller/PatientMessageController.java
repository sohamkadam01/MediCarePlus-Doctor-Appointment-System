...
import com.medicareplus.Service.PatientMessageService;
...
@RestController
@RequestMapping("/api/messages/patient")
@CrossOrigin("*")
...
public class PatientMessageController {
    ...
    public ResponseEntity<?> getThreads() {
        try {
            ...
            List<PatientMessageThreadDTO> threads = patientMessageService.getPatientThreads(patientId);
    ...
    public ResponseEntity<?> getThreadMessages(@PathVariable Long threadId) {
        try {
            Integer patientId = getCurrentUserId();
            List<MessageItemDTO> messages = patientMessageService.getThreadMessages(patientId, threadId);
    ...
    public ResponseEntity<?> sendMessage(@PathVariable Long threadId, @RequestBody SendMessageRequestDTO request) {
        try {
            Integer patientId = getCurrentUserId();
            MessageItemDTO sent = patientMessageService.sendPatientMessage(patientId, threadId, request.getText());
    ...
    public ResponseEntity<?> markThreadRead(@PathVariable Long threadId) {
        try {
            Integer patientId = getCurrentUserId();
            patientMessageService.markThreadAsRead(patientId, threadId);
